...
def extract_vertices_and_faces(fn):
    ifc = ifcopenshell.open(fn)

    settings = geom.settings()
    settings.set(settings.USE_WORLD_COORDS, True)

    elements = [e for e in ifc.by_type("IfcProduct") if e.Representation]
...
def main(files, dst_path):
    ...
    for fn in files:
        ...
        with (dst_path/"resolution_info_detailed.json").open("w") as f:
            json.dump(resolution_info, f, indent=2)
            


def make_dataset():
    src_path = Path("../../data/raw/IFCNetCore.IFC")
    dst_path = Path("../../data/processed/IFCGeomDetailed/IFCNetCore")
    files = sorted(src_path.glob("**/**/*.ifc"))
